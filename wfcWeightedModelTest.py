import random, anvil, time, math
from copy import deepcopy
from constantTable import MATRIX_X, MATRIX_Y, MATRIX_Z, EPOCH, EPSILON

"""
    WFC 가중치 모델 구현
    - WFC 모델과 기본 메커니즘은 동일
    - 패턴 추출 시 카운트 횟수에 따라 패턴 별 가중치 부여
    - 엔트로피 계산은 Shannon의 정의 활용
    - 노드 붕괴 시 가중치에 따라 붕괴할 패턴 결정
"""
    ...
    def collapse(self, x, y, z):
        ...
        for i in range(patternCount):
            if self.states[i] != 0:
                ...
                weights.append(PATTERNS[i].weight)
        if len(possibleStates) == 0: return
        collapsed_id = random.choices(possibleStates, weights=weights)[0]
        ...
        for i in range(patternCount):
            if self.states[i] != 0:
                if self.validatePattern(PATTERNS[i].state) == 0:
                    self.states[i] = 0
        self.updateEntropy()
        #entropyDelta = prevEntropy - self.updateEntropy()
...
class Pattern:
    def __init__(self, state = [], center_id = -1, weight = 0):
...
def findLeastEntropy():
    ...
    for i in range(SIZE_X):
        for j in range(SIZE_Y):
            for k in range(SIZE_Z):
                nodeEntropy = WFC_FIELD[i][j][k].getEntropy()
                coords.append((nodeEntropy, i, j, k))
    
    # 정렬 기준: patternEntropy, nodeEntropy
    coords.sort(key=lambda x:(x[0]))
    
    for e1, x, y, z in coords:
        if e1 > 0 and WFC_FIELD[x][y][z].collapsed == 0 and WFC_FIELD[x][y][z].isContradicted() == 0:
            return (x, y, z)
...
def WFCStep(step = -1, debug = 0, debugfile = None):
    ...
    # Debug
    if debug:
        e1 = WFC_FIELD[x][y][z].getEntropy()
        debugfile.write('{0:>3} {1:>3} {2:>3} {3:>5} {4:>10}\n'.format(x, y, z, step, round(e1, 6)))
        
        print('{0:>3} {1:>3} {2:>3} {3:>5} {4:>10}'.format(x, y, z, step, round(e1, 6)))
...
def runWFC(epoch, debug = 0):
    ...
    step = 1
    if debug:
        logfile = open('./Debug/wfcWeightedModelLog.txt', 'w+')
